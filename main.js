...
// Stable storage name, independent of the localized application name.
app.setPath('userData', path.join(app.getPath('appData'), 'macmic'));
...
const { DEFAULT_VOCABULARY } = require('./src/helpers/vocabulary');
if (databaseManager.getSetting('speech_vocabulary') === null) {
...
async function startApp() {
  ...
  // 记录系统信息
  logger.info('运行平台', { platform: process.platform, arch: process.arch });
  ...
  if (!databaseManager.getSetting('has_seen_macmic', false)) {
    await windowManager.showControlPanel();
    databaseManager.setSetting('has_seen_macmic', true);
  }
  logger.info('应用启动完成');
}
...
app.on('second-instance', () => windowManager.showControlPanel());
app.whenReady().then(() => {
...
app.on("activate", () => windowManager.showControlPanel());
